...
#version: 2019/01/09
#Copyright: Antoni Oliver (2019) - Universitat Oberta de Catalunya - aoliverg@uoc.edu
...
class TBXTools:
    ...
    #segmentadors i tokenitzadors
    
    def set_sl_tokenizer(self,tokenizerfile):
        self.sl_tokenizer=pickle.load(open(tokenizerfile,"rb"))
    ...
    def load_sl_corpus(self,corpusfile, encoding="utf-8",segment=True):
        ...
        for self.line in self.cf:
            if segment:
                #self.segments=self.sl_segmenter(self.line)
                self.segments=nltk.sent_tokenize(self.line)
            else:
                self.segments=[self.line]
            for self.line in self.segments:
                self.continserts+=1
            ...
            for self.s in self.cur.fetchall():
                ...
                if self.n>=self.n_min and self.n<=self.n_max and not self.candidate in self.knownterms:
                    ...
                    if show_measure and not show_frequency:
                        ...
                        self.cadena=str(self.n)+"\t"+str(self.measure)+"\t"+self.candidate
                    else:
                        self.cadena=self.candidate
                    if verbose:
                        #print(self.cadena)
        ...
        with self.conn:
            ...
            for self.s in self.cur.fetchall():
                ...
                for self.n in range(nmin,nmax+2): #we calculate one order bigger in order to detect nested candidates
                    ...
                    self.ngs=ngrams(nltk.word_tokenize(self.segment), self.n)
                    for self.ng in self.ngs:
                        self.ngrams[self.ng]+=1
                #for self.token in self.sl_tokenizer.tokenize(self.segment):
                for self.token in nltk.word_tokenize(self.segment):
    ...
    def regexp_exclusion(self,verbose=False):
        ...
        self.cur.execute("SELECT sl_exclusion_regexp FROM sl_exclusion_regexps")
        self.regexps=self.cur.fetchall()
        for self.r in self.regexps:
            self.nregexp=len(self.r[0].split(" "))
            self.exreg=self.r[0]
            #self.cur.execute("SELECT candidate FROM term_candidates where n<="+str(self.n_max))
            self.cur.execute("SELECT candidate FROM term_candidates where n="+str(self.nregexp))
            self.results=self.cur.fetchall()
    ...
    def association_measures(self,measure="raw_freq",n=2):
        ...
        for self.segment in self.cur.fetchall():
            ...
            self.textcorpus.extend(nltk.word_tokenize(self.segment[0]))
    ...
    def find_translation_ptable(self,sourceterm,numcandidates=5,maxdec=2,maxinc=2):
        ...
        return(self.results[0:numcandidates])
        
                
   
    def start_freeling_api(self,LANG):
        
        freelingpath='FreeLing-4.1/APIs/python'
    ...
    def tagged_ngram_calculation (self,nmin=2,nmax=3,minfreq=2):
        ...
        with self.conn:
            ...
            for self.s in self.cur.fetchall():
                ...
                for self.n in range(nmin,nmax+2): #we calculate one order bigger in order to detect nested candidates
                    #self.ngs=ngrams(word_tokenize(self.segment), self.n)
                ...
                if self.segment.find(slterm)>-1:
                    ...
                    for self.s2 in self.cur2.fetchall():
                        ...
                        for self.n in range(self.nmin,self.nmax+1):
                            ...
                            self.tlngs=ngrams(nltk.word_tokenize(self.tl_segment), self.n)
    ...
    #UNIGRAMS
    def select_unigrams(self,outputfile,position=1,encoding="utf-8"):
        self.cur.execute("SELECT candidate, n, frequency FROM term_candidates")
        self.results=self.cur.fetchall()
        self.tcaux={}
        for self.a in self.results:
            self.unigram=self.a[0].split(" ")[position]
            if self.unigram in self.tcaux:
                self.tcaux[self.unigram]+=self.a[1]
            else:
                self.tcaux[self.unigram]=self.a[1]
        self.sorted_x = sorted(self.tcaux.items(), key=operator.itemgetter(1),reverse=True)
        self.output=codecs.open(outputfile,"w",encoding=encoding)
        for self.s in self.sorted_x:
            cadena=str(self.s[1])+"\t"+self.s[0]
            #print(cadena)
...
#TSR
    ...
    def tsr(self,termsfile,encoding="utf-8", type="strict",max_iterations=10000000000,nmin=2,nmax=2,verbose=False):
        self.component={}
        self.firstcomponent={}
        ...
        for self.linia in self.entrada:
            self.linia=self.linia.rstrip()
            self.tsrterms.append(self.linia)
            self.camps=self.linia.split(" ")
            if len(self.camps)>=nmin and len(self.camps)<=nmax:
                self.contterms+=1
                ...
                if len(self.camps)>=3:
                    for self.i in range(1,len(self.camps)-1):
                        self.middlecomponent[self.camps[self.i]]=1
                        self.component[self.camps[self.i]]=1
        if verbose:
            pass
            #print("TOTAL REFERENCE TERMS:",self.contterms)
            #print("TOTAL FT:",len(self.firstcomponent))
            #print("TOTAL LT:",len(self.lastcomponent))
            #print("TOTAL MT:",len(self.middlecomponent))
        self.new=True
        self.newcandidates=collections.OrderedDict()
        self.hashvalue={}
        self.iterations=0
        while self.new:
            ...
            for self.r in self.results:
                ...
                self.rcamps=self.r[0].split(" ")
                if type=="strict":
                    #if self.rcamps[0] in self.firstcomponent and self.rcamps[1] in self.lastcomponent:
                    self.TSRInclude=True
                    for self.TSRposition in range(0,len(self.rcamps)):
                        if self.TSRposition==0:
                            if not self.rcamps[self.TSRposition] in self.firstcomponent:
                                self.TSRInclude=False
                        elif self.TSRposition==len(self.rcamps)-1:
                            if not self.rcamps[self.TSRposition] in self.lastcomponent:
                                self.TSRInclude=False
                        else:
                            if not self.rcamps[self.TSRposition] in self.middlecomponent:
                                self.TSRInclude=False
                    if self.TSRInclude:
                        if not self.r[0] in self.newcandidates:
                            self.newcandidates[self.r[0]]=self.r[1]
                            self.hashvalue[self.r[0]]=max_iterations-self.iterations
                            if not self.r[0] in self.tsrterms:
                                self.contdiscoberedterms+=1
                            self.new=True
                            #self.firstcomponent[self.rcamps[0]]=1
                            #self.lastcomponent[self.rcamps[-1]]=1
                            for self.TSRposition in range(0,len(self.rcamps)):
                                if self.TSRposition==0:
                                    self.firstcomponent[self.rcamps[self.TSRposition]]=1
                                elif self.TSRposition==len(self.rcamps)-1:
                                    self.lastcomponent[self.rcamps[self.TSRposition]]=1
                                else:
                                    self.middlecomponent[self.rcamps[self.TSRposition]]=1
                elif type=="flexible":
                    #if self.rcamps[0] in self.component or self.rcamps[1] in self.component:
                    ...
                    for self.TSRposition in range(0,len(self.rcamps)):
                        if self.TSRposition==0:
                            if self.rcamps[self.TSRposition] in self.firstcomponent:
                                self.TSRInclude=True
                        elif self.TSRposition==len(self.rcamps)-1:
                            if self.rcamps[self.TSRposition] in self.lastcomponent:
                                self.TSRInclude=True
                        else:
                            if self.rcamps[self.TSRposition] in self.middlecomponent:
                                self.TSRInclude=True
                    if self.TSRInclude:
                        if not self.r[0] in self.newcandidates:
                            self.newcandidates[self.r[0]]=self.r[1]
            ...
            if verbose:
                print(type,self.iterations,self.new,"NEW CANDIDATES",self.contdiscoberedterms)
