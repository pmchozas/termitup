...
def haceJson(termino, idioma,targets):
    ...
    #print(reponse2)
    jsondump=json.dumps(reponse2)#se guarda todo en un archivo json
...
def getIate(target, item,leng, termino):
    defi=''
    term_val=''
    syn=''
    if(target in leng):
    ...
    return( defi, syn, target)




#funcion que introduce todo en un csv 
def resultsIate(termino, jsonlist, idioma, targets):
    ...
    if('items' in data.keys()):
        ...
        for item in range(len(term)):#en cada de los siguientes ciclos se va interactuando en el json para obtener lo necesario
            ...
            for target in targets:
                result.append(getIate(target,item, leng,termino))
    #print('--TERMINO: ', termino)
    for i in result:
    	if(i[0]!='' and i[1]!=''):
            definition=i[0]
            syns=i[1]
            end.append(definition+' - '+syns+' - '+i[2])
            #print(definition, syns)

    #print(termino, '---',definition,'----', syns)
    return(end)


def resultsEurovoc(termino, idioma, relation):
    results=[]
    for r in relation:
        uriTermino=getUriTerm(termino, idioma)
        uriRelation=getRelation(uriTermino, r) 
        name=getName(uriRelation, idioma)
...
def resultsSyns(idioma,termino,targets,context,nameapi):
    if(termino):
        answer=lexicalaSearch(idioma, termino)
        results=answer['n_results']
        if(results>0):
            definitions=definitionGet(answer)
            maximo=wsid(termino,context, definitions,nameapi)
            tradMax=traductionGet(maximo, targets)
            synsTrad=justSyn(tradMax)
            getsyn=synonymsGet(maximo)
            print(termino, maximo, getsyn, tradMax, synsTrad)
        else:
            print('Sin resultados de termino en lexicala')

			#fileout(termino, maximo, getsyn, tradMax, synsTrad)
    #return(termino, maximo, getsyn, tradMax, synsTrad)
...
def wsid(termIn, context,  definitions,nameapi):
    pesos=[]
    if(nameapi=='all'):
...
if(nameapi=='iate'):
    ...
    jsonlist=haceJson(termino, idioma,targets)
    imp=resultsIate(termino, jsonlist, idioma, targets)
    for i in imp:
        print(i)
	...
	print(resultsEurovoc(termino, idioma, relation))
elif(nameapi=='syns'):
	termino=args.sourceTerm
	idioma=args.lang
	targets=args.targets.split(' ')
	context=args.context
	resultsSyns(idioma,termino,targets,context,nameapi)
elif(nameapi=='all'):
    relation=['broader', 'narrower', 'related']
    termino=args.sourceTerm
    idioma=args.lang
    ...
    jsonlist=haceJson(termino, idioma,targets)
    iate=resultsIate(termino, jsonlist, idioma, targets)
    eurovoc=resultsEurovoc(termino, idioma, relation)
    syns=resultsSyns(idioma,termino,targets,context,nameapi)
    print(iate)
    print(eurovoc)
    print(syns)
...
##IATE
##python3 searchTerm_all.py "maternity leave" en --targets "en es de nl" iate
##EUROVOC
#python3 searchTerm_all.py "maternity leave" en eurovoc
##LEXICA WSID
#python3 searchTerm_all.py "maternity leave" en --targets "en es de nl" --context "period in which a woman is legally allowed to be absent from work in the weeks before and after she gives birth" syns
#python3 searchTerm_all.py "descanso" es --targets "en de nl es" --context "Dicha distribución deberá respetar en todo caso los periodos mínimos de descanso diario y semanal previstos en la ley y el trabajador deberá conocer con un preaviso mínimo de cinco días el día y la hora de la prestación de trabajo resultante de aquella." syns
##ALL
#python3 searchTerm_all.py descanso es --targets "en de nl es" all
